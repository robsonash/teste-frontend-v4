...
const EQUIPMENT_POSITION_HISTORY = [
    {
...
class EquipmentService {
    ...
    findById(id: string): IEquipment {
        const response = EQUIPMENTS.find((row) => row.id === id)
        if (!response) {
            throw Error(`Equipement with id=${id} ENOT FOUND`)
        }
        return response
    }

    findPositionsById(id: string): IEquipmentPosition[] {
        const response = EQUIPMENT_POSITION_HISTORY.find((row) => row.equipmentId === id)
        if (!response) {
            throw Error(`Equipement with id=${id} POSITIONS NOT FOUND`)
        }
        return response.positions
    }

    findStatesById(id: string): IEquipmentState[] {
        const response = EQUIPMENT_STATE_HISTORY.find((row) => row.equipmentId === id)
        if (!response) {
            throw Error(`Equipement with id=${id} STATES NOT FOUND`)
        }
